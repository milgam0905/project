...
namespace WorldGeometry
{
    ...
    inline RenderPoint Project(RenderPoint p) { return {(p.x-p.y)*42.f,(p.x+p.y)*21.f}; }
    ...
    struct Trail { RenderPoint a,b,c,d; float width; };
    ...
    {
        for(const Trail& trail:Trails()) for(int i=0;i<=32;++i)
        {
